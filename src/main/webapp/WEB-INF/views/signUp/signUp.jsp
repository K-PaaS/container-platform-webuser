...
<script type="text/javascript">
    ...
    $('#registerBtn').click(function(){
        ...
        // user id
        if (!manCheck && commonUtils.isBlank($("#userId").val())) {
            msg = "아이디를 입력하세요.";
            manCheck = true;
        }
        // user id 정규식 검사
        if(!manCheck && commonUtils.regexIdPwd($("#userId").val())) {
            msg = "아이디는 영문 소문자로 시작하는 8자 이내의 영문 소문자, 숫자만 혼합하여 사용 가능합니다.";
            manCheck = true;
        }
        // 비밀번호
        if (!manCheck && commonUtils.isBlank($("#password").val())) {
            msg = "비밀번호를 입력하세요.";
            manCheck = true;
        }
        // 비밀번호 정규식 검사
        if (!manCheck && commonUtils.regexIdPwd($("#password").val())) {
            msg = "비밀번호는 영문 소문자로 시작하는 8자 이내의 영문 소문자, 숫자만 혼합하여 사용 가능합니다.";
            manCheck = true;
        }
        // 비밀번호 확인
        ...
        // 메일주소
        if (!manCheck && commonUtils.isBlank($("#email").val())) {
            msg = "메일 주소를 입력하세요.";
            manCheck = true;
        }

        // 메일주소
        if (!manCheck && commonUtils.regexEmail($("#email").val())) {
